...
export const SaveItem = async (req, res) => {
  try {
    ...
    const Item = await ListingModel.findById(id);
    const hasSaved = Item.likes.get(userId);
    if (hasSaved) {
      Item.likes.delete(userId);
    } else {
      Item.likes.set(userId);
    }
    const SavedListing = await ListingModel.findByIdAndUpdate(
      id,
      { likes: Item.likes },
      { new: true }
    );
